...
export default class PickerAndroid extends Component{
  ...
  _renderItems(items){
    ...
    items.forEach((item, index) => {
      ...
      upItems[index] = <TouchableHighlight
        key={'up'+index}
        onPress={() => {
          this._moveTo(index);
        }}
      ...
      middleItems[index] = <TouchableHighlight
        key={'mid'+index}
				style={{width}}
				onPress={() => {
        this._moveTo(index);
				}}><Text style={[styles.middleText, this.state.itemStyle]}>{item.label}</Text>
			</TouchableHighlight>;

      downItems[index] = <TouchableHighlight
        key={'down'+index}
        style={{width}}
        onPress={() => {
          this._moveTo(index);
        }}><Text style={[styles.downText, this.state.itemStyle]}
        >{item.label}
      </Text></TouchableHighlight>;
